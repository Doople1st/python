...
train_label_file = open("train_label_per_signer.txt","r")
test_label_file = open("test_validation_label_signer.txt","r")
validation_label_file = open("test_validation_label_signer.txt","r")
...
train_labels = keras.utils.to_categorical(np.array(train_raw_labels),num_classes = 100)
test_labels = keras.utils.to_categorical(np.array(test_raw_labels),num_classes = 100)
validation_labels = keras.utils.to_categorical(np.array(validation_raw_labels),num_classes = 100)
  ...
  if type=="train":
    while True:
      ...
      for i in range(BATCH):
        batch_features[i]= hf["train"][counter%300]
        batch_labels[i] = train_labels[counter%300]
    ...
    while True:
      ...
      for i in range(1):
        batch_features[i]= hf["test"][counter%100]
        batch_labels[i] = test_labels[counter%100]
        # print("Index: "+str(i))
        # print(batch_labels)
        counter+=1
      yield batch_features,batch_labels
    ...
    while True:
      ...
      batch_labels = np.zeros((BATCH,NUM_CLASSES))
      for i in range(BATCH):
        batch_features[i]= hf["validation"][counter%100]
        batch_labels[i] = validation_labels[counter%100]
        # print("Index: "+str(i))
        # print(batch_labels)
        counter+=1
      yield batch_features,batch_labels
...
best_checkpoint = ModelCheckpoint('per_signer_weights_best.hdf5', monitor='val_acc', verbose=1, save_best_only=True, mode='max')
checkpoint = ModelCheckpoint('per_signer_weights_epoch.hdf5', monitor='val_acc', verbose=1, save_best_only=False, mode='max')
csv_logger = CSVLogger('training_per_signer.log', append=False)
tensorboard = TensorBoard(log_dir='./per_signer_tf-logs')
