...
train_label_file = open("train_labels_4_per_signer.txt","r")
test_label_file = open("test_validation_4_per_signer.txt","r")
validation_label_file = open("test_validation_4_per_signer.txt","r")
...
best_checkpoint = ModelCheckpoint('4_per_signer_weights_best.hdf5', monitor='val_acc', verbose=1, save_best_only=True, mode='max')
checkpoint = ModelCheckpoint('4_per_signer_weights_epoch.hdf5', monitor='val_acc', verbose=1, save_best_only=False, mode='max')
csv_logger = CSVLogger('4_training_per_signer.log', append=False)
tensorboard = TensorBoard(log_dir='./4_per_signer_tf-logs')
callbacks_list = [checkpoint,best_checkpoint, csv_logger, tensorboard]

#len(hf["train"])
#len(hf["validation"])
rgb_model.fit_generator(generator("train"), steps_per_epoch=120//BATCH, epochs=30, callbacks=callbacks_list,shuffle=True,validation_data = generator("validation"),validation_steps=40//BATCH)

score = rgb_model.predict_generator(generator("test"),steps=40)
np.save("4_per_signer_result",score)
