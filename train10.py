...
def generator(type):
  ...
  if type=="train":
    while True:
      batch_features[i] = hf["train"][i%300]
      batch_labels[i] = train_labels[i%300]
      yield batch_features,batch_labels
      i+=1
  elif type=="test":
    batch_features[i] = hf["test"][i%300]
    batch_labels[i] = test_labels[i%300]
    yield batch_features,batch_labels
    i+=1
  elif type=="validation":
    batch_features[i] = hf["validation"][i%300]
    batch_labels[i] = validation_labels[i%300]
    yield batch_features,batch_labels
    i+=1
...
rgb_model.fit_generator(generator("train"), steps_per_epoch=64, epochs=100, callbacks=callbacks_list, verbose=1,shuffle=True,validation_data = generator("validation"),validation_steps=21)

score = rgb_model.evaluate_generator(generator("test"),steps=100)
